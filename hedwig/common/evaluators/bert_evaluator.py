...
class BertEvaluator(object):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating", disable=silent):
            ...
            if output_preds is None:
                ...
                if self.args.is_multilabel:
                    ...
                    target_labels = torch.argmax(label_ids, dim=1).cpu().detach().numpy()
            else:
                output_preds = np.append(output_preds, logits.cpu().detach().numpy(), axis=0)
                if self.args.is_multilabel:
                    target_labels = np.append(target_labels, label_ids.cpu().detach().numpy(), axis=0)
                else:
                    target_labels = np.append(target_labels, torch.argmax(label_ids, dim=1).cpu().detach().numpy(), axis=0)
    ...
    def get_accuracy(self, final_pred, label, silent=False):
        # if self.args.is_hierarchical:
        #     eval_features = convert_examples_to_hierarchical_features(
        #         self.eval_examples, self.args.max_seq_length, self.tokenizer)
        # else:
        ...
        # predicted_labels, target_labels = np.array(predicted_labels), np.array(target_labels)
        logits = torch.from_numpy(final_pred).float().to(self.args.device)
        if self.args.is_multilabel:
            predicted_labels = F.sigmoid(logits).round().long().cpu().detach().numpy()
            target_labels = label
        else:
